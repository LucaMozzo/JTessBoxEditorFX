 ...
 * the License.
 */
package com.github.tessboxeditor.utilities;
...
import static com.github.vietocr.util.Utils.readTextFile;
import static com.github.vietocr.util.Utils.writeTextFile;
...
//                boolean fontNameExist = lines.stream().anyMatch((s) -> s.startsWith(fontName + " ")); // Java 8
    ...
    public static void removeEmptyBoxes(File boxFile) {
        try {
            com.github.vietocr.util.Utils.writeTextFile(boxFile, com.github.vietocr.util.Utils.readTextFile(boxFile).replaceAll("(?m)^\\s+.*\n", ""));
        } catch (Exception e) {
            logger.log(Level.WARNING, e.getMessage(), e);
        }
    }
